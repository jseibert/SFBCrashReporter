...
void AppendKeyValueDataToResult(NSString *key, id value, NSMutableData *result, NSString *boundary);
...
void AppendKeyValueDataToResult(NSString *key, id value, NSMutableData *result, NSString *boundary) {
	...
	// Array value
		...
		for (id innerValue in (NSArray *)value) {
			AppendKeyValueDataToResult([key stringByAppendingString:@"[]"], innerValue, result, boundary);
		}
	}
	// Illegal class
	else
...
NSData * GenerateFormData(NSDictionary *formValues, NSString *boundary) {
	...
	for(NSString *key in keys) {
		...
		AppendKeyValueDataToResult(key, value, result, boundary);
	}
